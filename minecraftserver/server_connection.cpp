...
void proccessPacket(QUEUED_CONNECTION_CONTEXT* context, DWORD bytesRead) {
    ...
    int packetSize = 0, byte_position, packetid = 0;

    byte_position = readVarInt(context->buffer.buf, &packetSize, 0) + 1;
    ...
    byte_position = readVarInt(context->buffer.buf, &packetid, byte_position) + 1;
    invokePacket(packetid, context->buffer.buf, byte_position, context);
}

bool sendDataToConnection(char* buffer, int len, QUEUED_CONNECTION_CONTEXT* context) {
